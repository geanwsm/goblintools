...
- **No-space extraction no longer reads as a cipher**: a PDF text layer that drops the spaces between words concatenates a whole clause into one CamelCase token (`NotadaPropostaFinanceira`, `ModelodeProposta`, `TenhamcontrariadoaLegislacao`) — the first internal capital tripped `_is_cipher_token` and flagged perfectly good Portuguese as corrupt. New `_looks_like_glued_words` decomposes such tokens at the lowercase→UPPERCASE boundaries and, if they split into ≥2 word-shaped runs (plausible Portuguese onset, healthy vowel ratio, no 5+ consonant run, no mid-word capital accented vowel), treats them as layout — not a per-glyph cipher. Wordlist-free by design: the embedded list lacks common inflected forms (`proposta`, `anexo`, `financeira`). Verified on production editais 19332563 / 19323311 / 19331819 (now clean) with every known-corrupt document still detected.
- **Detector ignores URL slugs**: the substitution-cipher heuristic now NFKC-folds its input (`ﬁ`→`fi`, `²`→`2`, `º`→`o`) and strips `http(s)://…` / `www.…` before tokenizing, so a Banco de Preços receipt whose noise lives inside a `?token=v6iHJuY…` query string (bidding 19332500) no longer flags. `TextExtractor` output for clean PDFs is unchanged — normalization applies only to the text the detector inspects.
- **`ExtractionReport` reports a fully OCR-recovered scan as `clean`**: when every flagged page was re-read end to end by OCR and none stayed `corrupt_unrecoverable`, `overall_status` is now `clean` instead of `partially_recovered` (`used_ocr` still records that OCR ran, and a single `INFO` log notes it). Removes the bulk of the low-value `partially_recovered` warnings on plain scanned documents. **Behaviour change**: consumers keying on `overall_status == "partially_recovered"` for scanned PDFs should check `used_ocr` instead.
